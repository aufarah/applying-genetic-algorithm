...
def havesex(population):
    for i in range(numChild):
        ...
        constnar = np.random.uniform(-z,z)
        child.weights2[np.random.randint(limxW1)][chosen]   = constnar = np.random.uniform(0,2) * population[p2].weights2[np.random.randint(limxW1)][chosen]
        
        limxW1 = child.bias2.shape[0]
        limyW1 = child.bias2.shape[1]
        chosen= np.random.choice(limxW1,1,replace=False)
        constnar = np.random.uniform(-z,z)
        child.bias2[chosen][np.random.randint(limyW1)]      = constnar = np.random.uniform(0,2) * population[p2].bias2[chosen][np.random.randint(limyW1)]
